terraform {
  required_providers {
    kms = {
      # version = <latest version>
      source = "hanzokms/kms"
    }
  }
}

provider "kms" {
  host = "https://kms.hanzo.ai" # Only required if using self hosted instance of Hanzo KMS, default is https://kms.hanzo.ai
  auth = {
    universal = {
      client_id     = "<machine-identity-client-id>"
      client_secret = "<machine-identity-client-secret>"
    }
  }
}

# Request a certificate
resource "kms_cert_manager_certificate" "my_cert" {
  profile_id          = "<internal-ca-profile-id>"
  common_name         = "api.example.com"
  alt_names           = ["api.example.com", "api-internal.example.com"]
  organization        = "Example Corp"
  country             = "US"
  key_algorithm       = "RSA_2048"
  signature_algorithm = "RSA-SHA256"
  ttl                 = "90d"
  timeout_seconds     = 300
}


# Request a certificate using a CSR file
resource "kms_cert_manager_certificate" "csr_based_cert" {
  profile_id      = "<profile-id>"
  csr             = file("./my-certificate.csr")
  ttl             = "90d"
  timeout_seconds = 300
}

# Request a certificate using an inline CSR
resource "kms_cert_manager_certificate" "inline_csr_cert" {
  profile_id = "<profile-id>"

  csr = <<-CSR
-----BEGIN CERTIFICATE REQUEST-----

0A7YVKj492nqnN/xS5kzIidZuaCBLocLo5j615xh/79YgMZjrGo3wvnk
-----END CERTIFICATE REQUEST-----
CSR

  ttl             = "90d"
  timeout_seconds = 300
}

# Outputs
output "cert_details" {
  value = {
    id            = kms_cert_manager_certificate.my_cert.id
    status        = kms_cert_manager_certificate.my_cert.status
    serial_number = kms_cert_manager_certificate.my_cert.serial_number
    not_before    = kms_cert_manager_certificate.my_cert.not_before
    not_after     = kms_cert_manager_certificate.my_cert.not_after
  }
}

output "cert_pem" {
  value = kms_cert_manager_certificate.my_cert.certificate
}

output "cert_chain" {
  value = kms_cert_manager_certificate.my_cert.certificate_chain
}

output "cert_private_key" {
  value     = kms_cert_manager_certificate.my_cert.private_key
  sensitive = true
}
