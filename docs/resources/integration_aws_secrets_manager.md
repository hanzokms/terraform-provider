...
page_title: "kms_integration_aws_secrets_manager Resource - terraform-provider"
subcategory: "Native Integrations - DEPRECATED"
description: |-
  Deprecated: This resource is deprecated and will be removed in a future version. Use kms_secret_sync_aws_secrets_manager instead.
  Create AWS Secrets Manager integration & save to Hanzo KMS. Only Machine Identity authentication is supported for this resource.
---

# kms_integration_aws_secrets_manager (Resource)
...
- `metadata_sync_mode` (String) The sync mode for AWS tags. The supported options are `secret-metadata` and `custom`. If `secret-metadata` is selected, the metadata of the Hanzo KMS secrets are used as tags in AWS (only supported for one-to-one integrations). If `custom` is selected, then the key/value pairs in the `aws_tags` field is used.
