...
resource "kms_cert_manager_ca_certificate" "root_cert" {
  ca_id = kms_cert_manager_internal_ca.root.id
...
resource "kms_cert_manager_ca_certificate" "issuing_cert" {
  ca_id        = kms_cert_manager_internal_ca.issuing.id
  parent_ca_id = kms_cert_manager_internal_ca.root.id
  ...
  depends_on = [kms_cert_manager_ca_certificate.root_cert]
}

# Example with shorter validity period for testing/development
resource "kms_cert_manager_ca_certificate" "dev_cert" {
  ca_id = kms_cert_manager_internal_ca.root.id
...
output "root_ca_certificate" {
  ...
  value       = kms_cert_manager_ca_certificate.root_cert.certificate
  sensitive   = true
}

output "root_ca_serial_number" {
  description = "The serial number of the root CA certificate"
  value       = kms_cert_manager_ca_certificate.root_cert.serial_number
}

output "issuing_ca_certificate" {
  description = "The issuing CA certificate"
  value       = kms_cert_manager_ca_certificate.issuing_cert.certificate
  sensitive   = true
}

output "issuing_ca_chain" {
  description = "The issuing CA certificate chain"
  value       = kms_cert_manager_ca_certificate.issuing_cert.certificate_chain
  sensitive   = true
}
